...
def main():
    ...
    model = SearchCNNController(input1_channels, input2_channels, config.init_channels, n1_classes, n2_classes,
                                config.layers,
    ...
    for epoch in range(config.epochs):
        ...
        # training
        train_step(
            train1_loader, valid1_loader, train2_loader, valid2_loader,
            model, architect, w1_optim, w2_optim, alpha_optim, lr1, lr2, epoch)

        # validation
        cur_step = (epoch + 1) * len(train1_loader)
        sub1top1, sub2top1, loss = validate(valid1_loader, valid2_loader, model, epoch, cur_step)
    ...
    logger.info("Final best Prec1@1 = {:.4%} Prec2@1 = {:.4%}".format(best_sub1top1, best_sub2top1))
    logger.info("Best Genotype = {}".format(best_genotype))


def train_step(train1_loader, valid1_loader, train2_loader, valid2_loader, model, architect, w1_optim, w2_optim,
               alpha_optim, lr1, lr2, epoch):
    ...
    for step, ((trn1_X, trn1_y), (val1_X, val1_y), (trn2_X, trn2_y), (val2_X, val2_y)) in enumerate(
            zip(train1_loader, valid1_loader, train2_loader, valid2_loader)):
    ...
    logger.info("Train: [{:2d}/{}] Final Prec1@1 {:.4%} Prec2@1 {:.4%}".format(epoch + 1, config.epochs, sub1top1.avg,
                                                                               sub2top1.avg))

def validate(valid1_loader, valid2_loader, model, epoch, cur_step):
    sub1top1 = utils.AverageMeter()
    sub1top5 = utils.AverageMeter()
    sub1losses = utils.AverageMeter()
    ...
    with torch.no_grad():
        for step, ((X1, y1), (X2, y2)) in enumerate(zip(valid1_loader, valid2_loader)):
    ...
    logger.info("Valid: [{:2d}/{}] Final Prec1@1 {:.4%} Prec2@1 {:.4%}".format(epoch + 1, config.epochs, sub1top1.avg,
                                                                               sub2top1.avg))

    return sub1top1.avg, sub2top1.avg, sub1losses.avg + sub2losses.avg
